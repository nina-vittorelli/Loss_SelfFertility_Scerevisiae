...
path_artificial_HO_deletion <- "03_output/01_cleaned_panels/02_strains_with_artificial_HO_deletion_1011/strains_with_artificial_HO_deletion_1011.txt"
...
# tables
HO_variants_3034_path <- "03_output/tables/supptable4_3034GP_HO_variants.csv"
table_3039_with_analyses_path <- "03_output/tables/supptable5_3034GP_strains_thallism.csv"

# figures
fig3_path <- "03_output/figures/figure_3.pdf"
suppfig6_path <- "03_output/figures/suppfigure_6.pdf"
...
## Artificial HO deletion
```{r}
strains_with_artificial_HO_deletion <- readLines(path_artificial_HO_deletion)
```
...
```{r}
# clean
remove(path_3039_db, path_artificial_HO_deletion, strains_with_artificial_HO_deletion)
```
...
ggsave(suppfig6_path, width = 6, height = 8)
```
...
p_genotype_frequency_isolates <- ggplot(table_genotypes) +
  geom_histogram(aes(x = number_isolates, fill = factor(predicted_functionality, levels = c("unknown", "non-functional", "functional"))), ) +
  scale_fill_manual(values = c("functional" = "#c2f9c9" , "non-functional" = "#661100", "heterozygous_non-functional"  = "#f89c4f", "unknown" = "gray90")) +
  myTheme() +
...
table_genotypes_sumup_functionality <-  table_genotypes %>%
  ...
  mutate(total_number_genotypes = n(),
    ...
    total_isolates = paste(str_split(sequences, "\\, ") %>% unlist() %>% str_split_i("_YDL", 1) %>% unique() %>% sort(), collapse = ", "),
    total_number_isolates = str_split(sequences, "\\, ") %>% unlist() %>% str_split_i("_YDL", 1) %>% unique() %>% length(), 
...
fig4A <- ggplot(thallism_for_plots) +
  geom_bar(aes(x = factor(Ploidy, levels = c("1","2","3","4","5")), 
               fill = factor(thallism, levels=c("unknown", "heterothallic", "homothallic")), 
              )) + 
  scale_fill_manual(values=c("homothallic" = "#c2f9c9" , "heterothallic" = "#661100", "unknown" = "grey90"), na.value = "transparent") +
  myTheme() + 
...
```{r}
thallism_3039_for_mosaic <- thallism_for_plots %>%
  filter(!is.na(Zygosity),
         !is.na(Clade)) %>%
  mutate(Zygosity = factor(Zygosity, levels = c("Homozygous", "Heterozygous")))
nrow(thallism_3039_for_mosaic)
```


## Figure 4B
```{r}
intersection_count = thallism_3039_for_mosaic %>%
  group_by(Zygosity) %>%
  mutate(prop_zygosity = n() / nrow(thallism_3039_for_mosaic)) %>%
  ungroup() %>%
  group_by(Zygosity, thallism, prop_zygosity) %>%
  mutate(prop_zygthall = n() / (nrow(thallism_3039_for_mosaic) * prop_zygosity)) %>%
  select(Zygosity, thallism, prop_zygosity, prop_zygthall) %>%
  unique() %>%
  pivot_wider(names_from = thallism, values_from = prop_zygthall)  %>%
  mutate(unknown =  heterothallic + homothallic + (unknown / 2), 
         homothallic = heterothallic + (homothallic / 2),
         heterothallic =  heterothallic / 2) %>%
  pivot_longer(cols = c( "heterothallic", "homothallic", "unknown"), names_to = "thallism", values_to = "pos_y") %>%
  mutate(pos_x = case_when(
    Zygosity == "Heterozygous" ~ (1-prop_zygosity) + (prop_zygosity /2), 
    TRUE ~ prop_zygosity /2
  )) %>%
  select(Zygosity, thallism, pos_x, pos_y)

homozygous_pos = mean(intersection_count$prop_zygosity[intersection_count$Zygosity == "Homozygous"]) / 2
heterozygous_pos = (2*homozygous_pos)  + (mean(intersection_count$prop_zygosity[intersection_count$Zygosity == "Heterozygous"]) / 2)
  
fig4B <- ggplot(thallism_3039_for_mosaic %>% mutate(pop = "All strains")) +
  geom_mosaic(aes(x = product(thallism, Zygosity), 
              fill = thallism), 
              color = "black") +
  scale_fill_manual(values=c("homothallic" = "#c2f9c9" , "heterothallic" = "#661100", "unknown" = "grey90"), na.value = "transparent") +
    facet_wrap(pop ~ .)   +
    annotate(geom = "text", x = 0.5, y = 1.1, 
             label = paste0("n=", 
                             nrow(thallism_3039_for_mosaic), 
                            ", ", 
                            100 * round(sum(thallism_3039_for_mosaic$Ploidy > 2, na.rm = TRUE) / nrow(thallism_3039_for_mosaic), 2), 
                            "% polyploids"), size = 4, color = "grey30", face = "bold") +
  annotate(geom = "text", x = homozygous_pos, y = -0.1, label = "%homozygous", size = 3, color = "grey30") +
  annotate(geom = "text", x = heterozygous_pos, y = -0.1, label = "%heterozygous", size = 3, color = "grey30") +
  ylab("")+
...
thallism_for_plots %>% 
  ...
  mutate(total = Homozygous + Heterozygous + `NA`, 
...
```{r}
polyploidy_proportion <- thallism_3039_for_mosaic %>% 
  mutate(ploidy = case_when( 
    Ploidy > 2 ~"polyploid", 
    TRUE ~ "other"
  )) %>%
...
fig4C <- ggplot(thallism_3039_for_mosaic) +
  geom_mosaic(aes(x = product(thallism, Zygosity), 
              fill = thallism), 
              color = "black") +
...
```{r}
homothallic_clades <- thallism_for_plots %>% 
...
```{r}
heterothallic_clades <- thallism_for_plots %>% 
  group_by(Clade, thallism) %>% 
  summarise(n = n()) %>% 
  pivot_wider(names_from = thallism, values_from = n, values_fill = 0) %>%
  mutate(total = heterothallic + homothallic + unknown, 
...
```{r}
both_clades <- thallism_for_plots %>% 
  group_by(Clade, thallism) %>% 
  summarise(n = n()) %>% 
  pivot_wider(names_from = thallism, values_from = n, values_fill = 0) %>%
  mutate(total = heterothallic + homothallic + unknown, 
...
figure_4 <- plot_grid(fig4ABC, plot_spacer_clean, fig4D, ncol = 1, labels = c("", "", "D"), rel_heights = c(5, 0.1, 1))


figure_4

ggsave(fig4_path, width = 8, height = 15)
```
