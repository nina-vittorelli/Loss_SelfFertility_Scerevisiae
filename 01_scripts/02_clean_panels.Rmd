...
# tables
path_assemblies_with_artificial_deletion_df <- "03_output/tables/Dataset_S2_scrap_assemblies_with_markers.csv"

# figures
path_plot_type_ploidy_zygosity_scrap <- "03_output/figures/Figure_S1.pdf"
path_plot_ploidy_zygosity_3039 <- "03_output/figures/Figure_S8.pdf"
...
artificial_cassettes_df %>%
  arrange(contiguity)
```
Manual checks for the two assemblies with putative non-contiguous cassettes: 
...
```
```{r}
 # remove strains with missing information about zygosity
strains_with_missing_Zygosity <- df_3039_raw$StandardizedName[df_3039_raw$Zygosity == "NA"]
strains_with_missing_Zygosity
strains_to_exclude <- c(strains_with_HO_deletion, strains_with_missing_Zygosity)

writeLines(strains_to_exclude, path_strains_to_exclude)
```

```{r}
# remove 
df_3039 <- df_3039_raw  %>%
  filter(!(StandardizedName %in% strains_to_exclude)) %>%
