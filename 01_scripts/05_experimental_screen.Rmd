...
# figures
suppfig2_path <- "03_output/figures/suppfigure_2.pdf"
...
-   2 monosporic isolates are haploid according to ScRAP metadata, but they are a/alpha and they sporulate (CHS_3a and CPI_1c). Among our hypothesis: (1) our tube was contaminated or (2) they underwent rare mating-type switching between the sequencing and our experiments. We dissected new isolates from the same genetic background to conclude about these two isolates. For now, we consider their homothallism as dubious.
...
table_for_fig_1C <- scrap_df_4 %>%
  filter(thallism %in% c("mixed","heterothallic","homothallic"))
```
...
p_screen_result <- 
 ggplot(table_for_fig_1C %>%
         mutate(sequencing_category = case_when(
             ...
             TRUE ~ "32 original strains"))) +
  geom_bar(aes(x = factor(ploidy_scrap_isolate_corrected, levels = c(1,2,3,4)), fill = factor(thallism, levels=c("mixed","heterothallic","homothallic"))), color = "black") +
  facet_grid(sequencing_category ~ . ) +
  myTheme()  +
  scale_fill_manual(values=c("homothallic" = "#c2f9c9" , "mixed" = "#f89c4f", "heterothallic" = "#661100")) +
  labs( x = "ploidy", y = "number of strains", fill = "thallism") 

p_screen_result
```
```{r}
remove(new_MI_df, p_summary_mat_MI, p_summary_thallism_MI, spViability_per_tetrad_3d_df, scrapMI_OI_df, table_for_fig_1C)
```
...
ggsave(suppfig2_path, width = 11, height = 6)
```
...
ggsave(figure_1_path, figure_1, width = 15, height = 8)
```
