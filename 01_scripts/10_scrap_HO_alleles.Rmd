...
# tables
table_variants_HO_path <- "03_output/tables/Dataset_S3_scrap_HO_variants.csv"
supptable_1_path <- "03_output/tables/Dataset_S1_117_scrap.csv"

# figures
fig2_path <- "03_output/figures/Figure_2.pdf"
suppfig4_path <- "03_output/figures/Figure_S4.pdf"
suppfig5_path <- "03_output/figures/Figure_S5.pdf"
suppfig6_path <- "03_output/figures/Figure_S6.pdf"
...
### Figure S4
```{r}
suppfig4 <- plot_grid(var_freq_funct, var_cds_freq_funct_pos, labels = c("A", "B"), rel_heights = c(1, 1.5), ncol = 1)

suppfig4

ggsave(suppfig4_path, height = 6, width = 8)
...
```{r}
suppfig5down <- plot_grid(p_allele_frequency_seq, p_allele_frequency_isolates, labels = c("B", "C"),  ncol = 2)

suppfig5 <- plot_grid(p_allele_nb_variants, suppfig5down, labels = c("A", ""),  ncol = 1, rel_heights = c(1, 0.6))

ggsave(suppfig5_path, height = 10, width = 8)

remove(suppfig5down, suppfig4)
```

```{r}
# clean
remove(suppfig5, suppfig5_path, p_allele_frequency_seq, p_allele_frequency_isolates, p_allele_nb_variants)
```
...
```{r}
nonneutral_var_order_plot <- table_alleles %>%
  ...
  pivot_longer(cols = -id_allele, names_to = "id_variant", values_to = "presence") %>%
  group_by(id_variant) %>%
  summarise(nb = sum(presence)) %>%
  arrange(desc(nb))

nonneutral_var_order_plot <- nonneutral_var_order_plot$id_variant
```
...
table_nonfunct_alleles = table_alleles %>%
  ...
  # order by variants
   arrange(across(all_of(nonneutral_var_order_plot))) %>%
...
variants_for_tree <- table_variants_funct %>%
  
  # count the number of strains carrying the variant
  pivot_longer(cols = all_of(seqs_all), names_to = "seq", values_to = "presence") %>%
  mutate(strain = str_split_i(seq, "\\.", 1)) %>%
  group_by(id_var, strain) %>%
  ...
  # filter for variants carried by 5-50 strains
  filter(n > 4, n < 51) %>%
  ungroup() %>%
  mutate(position =  str_extract(id_var, "\\d+") %>% as.numeric()) %>%

  
  # arrange by position on the coding sequence
  arrange(position) 

variants_for_tree

variants_for_tree <- variants_for_tree$id_var
```
...
ggsave(suppfig6_path, height = 7, width = 12)
```
